  ...
  describe ".call" do
    ...
    context "when a digest run doesn't exist" do
      ...
      it "creates a DigestRunSubscriber for each subscription" do
        expect { described_class.call(range: Frequency::DAILY) }
          .to change { DigestRunSubscriber.exists?(subscriber_id: subscribers.map(&:id)) }
          .to(true)
      end

      it "enqueus DigestEmailGenerationWorker for each DigestRunSubscriber" do
        described_class.call(range: Frequency::DAILY)
        expect(DigestEmailGenerationWorker).to have_received(:perform_async).twice
      end
    end
